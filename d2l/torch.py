...
def plot(X, Y=None,
    ...
    :param xlim: x 轴范围
    :param ylim: y 轴范围
    :param xscale: x 轴缩放比例,一般是 linear 均匀分布
    :param yscale: y 轴缩放比例,一般是 linear 均匀分布
    :param fmts: 函数图像的格式,详情查看源码即可
    :param figsize: 试图大小
    :param axes: 使用传入的坐标系,否则自动建立新的坐标系
    """
    if legend is None:
        legend = []
...
def evaluate_accuracy(net, data_iter):
    ...
    if isinstance(net, torch.nn.Module):
        net.eval()  # Set the model to evaluation mode 将模型设置为评估模式
    metric = Accumulator(2)  # No. of correct predictions, no. of predictions 正确预测数、预测总数
